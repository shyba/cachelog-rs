...
pub async fn interactive_test_with_progress<D: Driver>(
    ...
    for run in 0..config.num_runs {
        ...
        let result = run_single_test(&mut driver, &ctx, &mut *rng, &mut stats).await;
...
async fn run_single_test<D: Driver>(
    ...
    let assume_result = ctx
        .client
        .assume_transition(ctx.session, init_idx, true)
        .await?;
    ...
    for step_idx in 1..ctx.config.max_steps {
        ...
        let Some(chosen_idx) = chosen else {
            debug!(
                run = ctx.run,
                step = step_idx,
                "No enabled transitions (deadlock)"
            );
...
fn compare_states<D: Driver>(
    ...
    let spec_state =
        D::State::from_spec(spec_itf_state).map_err(|e| StepError::SpecDeserialize {
            context: ctx.clone(),
            reason: e.to_string(),
        })?;
