...
pub fn validate_trace(
    ...
    let output =
        crate::util::run_with_timeout(&mut cmd, config.timeout).map_err(ValidationError::from)?;
...
fn parse_apalache_output(
    ...
    match exit_code {
        ...
        _ => {
            ...
            Err(
                ValidationError::from(crate::error::ApalacheError::Execution {
                    exit_code,
...
pub fn ndjson_to_tla_module(trace_file: &Path) -> Result<(String, usize), Error> {
    ...
    for (i, line) in content.lines().enumerate() {
        ...
        let obj: serde_json::Value =
            serde_json::from_str(line).map_err(|e| ValidationError::InvalidJson {
                line: line_num,
                reason: e.to_string(),
            })?;
        ...
        let obj_map = obj
            .as_object()
            .ok_or_else(|| ValidationError::NonObjectState {
        ...
        serde_json::Value::Object(obj) => {
            ...
            let fields: Result<Vec<String>, Error> = sorted
                ...
                .map(|(k, v)| {
                    Ok(format!(
                        "{k}: {}",
                        infer_snowcat_type(v, &format!("{field}.{k}"))?
                    ))
                })
                .collect();
            Ok(format!("{{{}}}", fields?.join(", ")))
...
fn json_obj_to_tla_record(value: &serde_json::Value, line: usize) -> Result<String, Error> {
    let obj = value
        .as_object()
        .ok_or_else(|| ValidationError::TlaConversion {
            line,
...
fn json_to_tla_value(value: &serde_json::Value, line: usize, field: &str) -> Result<String, Error> {
    match value {
        ...
        serde_json::Value::Object(_) => json_obj_to_tla_record(value, line),
    }
}
...
mod tests {
    ...
    fn json_to_tla_value_string() {
        assert_eq!(
            json_to_tla_value(&json!("hello"), 1, "f").unwrap(),
            "\"hello\""
        );
    }
    ...
    fn json_to_tla_value_array() {
        assert_eq!(
            json_to_tla_value(&json!([1, 2, 3]), 1, "f").unwrap(),
            "<<1, 2, 3>>"
        );
