...
pub fn generate_traces(config: &ApalacheConfig) -> Result<GeneratedTraces, Error> {
    let (out_dir, temp) = match &config.out_dir {
        ...
        None => {
            let tmp = tempfile::tempdir().map_err(|e| TraceGenError::TempDir(e.to_string()))?;
    ...
    let output =
        crate::util::run_with_timeout(&mut cmd, config.timeout).map_err(TraceGenError::from)?;
...
fn collect_itf_traces(out_dir: &Path) -> Result<Vec<itf::Trace<itf::Value>>, Error> {
    ...
    for path in walkdir(out_dir)? {
        ...
        if filename.ends_with(".itf.json") {
            ...
            let content =
                std::fs::read_to_string(&path).map_err(|e| TraceGenError::TraceParse {
                    path: path.clone(),
                    reason: e.to_string(),
                })?;
...
mod tests {
    ...
    fn builder_with_spec_succeeds() {
        let config = ApalacheConfig::builder().spec("test.tla").build().unwrap();
...
fn walkdir(dir: &Path) -> Result<Vec<PathBuf>, Error> {
    ...
    for entry in std::fs::read_dir(dir).map_err(|e| {
        TraceGenError::from(crate::error::DirectoryReadError {
