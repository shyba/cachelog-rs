...
pub fn replay_traces_with_progress<D: Driver>(
    ...
    for (trace_idx, trace) in traces.iter().enumerate() {
        ...
        let states = replay_single_trace(&mut driver, trace, trace_idx, total_traces, &progress)?;
...
fn replay_single_trace<D: Driver>(
    ...
    for (state_idx, itf_state) in trace.states.iter().enumerate() {
        ...
        let (action_taken, nondet_picks) =
            extract_mbt_vars(state_value, &itf_state.meta).map_err(|reason| {
                ReplayError::MbtVarExtraction {
                    trace: trace_idx,
                    state: state_idx,
                    reason,
                }
        ...
        let driver_state = <D::State as ExtractState<D>>::from_driver(driver).map_err(|e| {
            StepError::DriverStateExtraction {
                context: ctx.clone(),
                reason: e.to_string(),
            }
        })?;
...
pub fn load_traces_from_dir(dir: &std::path::Path) -> Result<Vec<itf::Trace<itf::Value>>, Error> {
    ...
    for entry in std::fs::read_dir(dir).map_err(|e| {
        ReplayError::from(crate::error::DirectoryReadError {
            path: dir.to_path_buf(),
            reason: e.to_string(),
        })
    })? {
        ...
        if filename.ends_with(".itf.json") {
            let content = std::fs::read_to_string(&path).map_err(|e| {
                ReplayError::Parse(format!("Failed to read {}: {e}", path.display()))
            })?;
            let trace: itf::Trace<itf::Value> = serde_json::from_str(&content).map_err(|e| {
                ReplayError::Parse(format!("Failed to parse {}: {e}", path.display()))
            })?;
...
mod tests {
        ...
        let state = itf::Value::Record(
            vec![
                (
                    ...
                    itf::Value::String("increment".into()),
                ),
                (
                    "nondet_picks".to_string(),
                    itf::Value::Record(
                        vec![("amount".to_string(), itf::Value::Number(5))]
                            .into_iter()
                            .collect(),
                    ),
                ),
    ...
    fn extract_mbt_vars_prefers_meta_over_state_field() {
        let state = itf::Value::Record(
            vec![
                (
                    "action_taken".to_string(),
                    itf::Value::String("from_state".into()),
                ),
        ...
        let (action, _) =
            extract_mbt_vars(&state, &meta_with_action(1, "action", "from_meta")).unwrap();
    ...
    fn extract_mbt_vars_uses_label_key() {
        ...
        let (action, _) =
            extract_mbt_vars(&state, &meta_with_action(1, "label", "increment")).unwrap();
        assert_eq!(action, "increment");
    }
    ...
    fn extract_mbt_vars_uses_transition_key() {
        ...
        let (action, _) =
            extract_mbt_vars(&state, &meta_with_action(1, "transition", "decrement")).unwrap();
